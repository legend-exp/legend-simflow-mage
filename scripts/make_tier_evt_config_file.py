...
parser = argparse.ArgumentParser(
    description="""Generates config files for mage-post-proc. Code depends on legend-metadata, and looks for input excel sheet "fccd-reviewed.xlsx" in the same folder."""
)
parser.add_argument(
    "-d",
    "--dataset",
    type=str,
    default="golden",
    help="golden or silver dataset. Default:golden",
)
parser.add_argument(
    "-w", "--wildcard", type=str, help="Passing on the snakemake wildcard."
)
...
mirion_icpc = mirion_icpc.loc[mirion_icpc["det name"].isin(l140_det)].reset_index(
    drop=True
)
gerda_det = df2.loc[df2["det name"].isin(l140_det)].reset_index(drop=True)
ortec_icpc = df4.drop(columns=["fccd-mm (manufacturer)"])
ortec_icpc = ortec_icpc.loc[ortec_icpc["det name"].isin(l140_det)].reset_index(
    drop=True
)
...
parfile = glob.glob(
    f"/pscratch/sd/j/jita/jita/rushcalV3-mpp/pargen-parser/new-par-hit-results/cal/*/*/{wcard}*par_hit_results.json"
)[0]
...
for gedname in detlist:
    ...
    if ch in channels:
        ...
        outstr[mageid] = {
            ...
            "nplus-fccd-mm": fccd,
            "energy": {
                "sig0": math.sqrt(eres[0]) / 2.355,
                "sig1": math.sqrt(eres[1]) / 2.355,
                "sig2": 0,
            },
            "usability": mask,
        }
        ...
        outstr[mageid] = {
            "name": gedname,
            "nplus-fccd-mm": fccd,
            "energy": {"sig0": null, "sig1": null, "sig2": null},
            "usability": mask,
        }
